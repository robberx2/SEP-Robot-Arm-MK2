...
try:
    ...
    DEFAULT_SETTINGS = {
        ...
        "joint_max_speed": [90.0, 90.0, 90.0, 90.0],
        "joint_acceleration": [180.0, 180.0, 180.0, 180.0],
        "colors": {
            "panel": [0.04, 0.06, 0.10],
    ...
    def load_settings(path):
        ...
        for key, fallback in (("joint_max_speed", 90.0), ("joint_acceleration", 180.0)):
            values = settings.get(key, DEFAULT_SETTINGS[key])
            settings[key] = [max(fallback if key == "joint_acceleration" else 1.0, float(value)) for value in values[:4]]
            while len(settings[key]) < 4:
                settings[key].append(fallback)
        settings["colors"] = settings.get("colors", DEFAULT_SETTINGS["colors"])
        for color_name in ("panel", "accent"):
            values = settings["colors"].get(color_name, DEFAULT_SETTINGS["colors"][color_name])
            settings["colors"][color_name] = [
                max(0.0, min(1.0, float(value))) for value in values[:3]
            ]
        limits = settings.get("joint_limits", DEFAULT_SETTINGS["joint_limits"])
        settings["joint_limits"] = [
            [
                min(float(pair[0]), float(pair[1])),
                max(float(pair[0]), float(pair[1])),
            ]
            for pair in limits[:4]
        ]
        while len(settings["joint_limits"]) < 4:
            settings["joint_limits"].append(
                list(DEFAULT_SETTINGS["joint_limits"][len(settings["joint_limits"])])
            )
        return settings
...
def update_render_viewport():
    render_x = int(display_x * split_ratio)
    render_width = max(1, display_x - render_x)
    glViewport(render_x, 0, render_width, max(1, display_y))
    glMatrixMode(GL_PROJECTION)
    glLoadIdentity()
    gluPerspective(
        fov,
        render_width / max(1, display_y),
        cliping_plane_min,
        cliping_plane_max,
    )
    glMatrixMode(GL_MODELVIEW)
...
def resize_display(width, height):
    global display_x, display_y, aspect_ratio
    display_x = max(800, int(width))
    display_y = max(700, int(height))
    aspect_ratio = display_x / display_y
    pygame.display.set_mode(
        (display_x, display_y), DOUBLEBUF | OPENGL | RESIZABLE
    )
    text_texture_cache.clear()
    update_render_viewport()

def split_position():
    return int(display_x * split_ratio)
...
def draw_text(text, x, y, size=24, color=(255, 255, 255)):
    cache_key = (text, size, tuple(color))
    cached = cache_key in text_texture_cache
    if cached:
        tex_id, w, h = text_texture_cache[cache_key]
    else:
        font = font_cache.setdefault(size, pygame.font.SysFont(None, size))
        surface = font.render(text, True, color)
        tex_id = load_image_from_surface(surface)
        w, h = surface.get_size()
        text_texture_cache[cache_key] = (tex_id, w, h)
        if len(text_texture_cache) > 512:
            old_key = next(iter(text_texture_cache))
            old_tex_id, _, _ = text_texture_cache.pop(old_key)
            if old_tex_id != tex_id:
...
def solve_arm_ik(target, orientation, starting_angles, angle_weight):
    ...
    orientation_weight = max(0.0, min(1.0, float(angle_weight))) * 0.005
    ...
    for _ in range(160):
        ...
        damping = 0.08 if np.linalg.norm(position_error) > 0.05 else 0.15
        damped_inverse = np.linalg.solve(
            jacobian @ jacobian.T + damping ** 2 * np.eye(5),
            np.eye(5),
        )
        damped_pseudoinverse = jacobian.T @ damped_inverse
        correction = damped_pseudoinverse @ error
        null_space = np.eye(4) - damped_pseudoinverse @ jacobian
        ...
        if correction_norm > 8.0:
            correction *= 8.0 / correction_norm
        angles += correction
        for index, (low, high) in enumerate(joint_limits):
            angles[index] = np.clip(angles[index], low, high)
    for index, (low, high) in enumerate(joint_limits):
...
def slider_field_id(label):
    fixed = {
        "P": "p_selected",
        "I": "i_selected",
        "D": "d_selected",
        "Max speed": "max_speed",
        "Posture bias": "posture_bias",
        "Angle weight": "angle_weight",
        "Target X": "target_x",
        "Target Y": "target_y",
        "Target Z": "target_z",
        "End roll": "orientation_roll",
        "End pitch": "orientation_pitch",
        "End yaw": "orientation_yaw",
        "PID integral limit": "integral_limit",
        "PID output limit": "output_limit",
    }
    if label in fixed:
        return fixed[label]
    for index, joint_name in enumerate(joint_names):
        if label == f"{joint_name} min":
            return f"limit_low_{index}"
        if label == f"{joint_name} max":
            return f"limit_high_{index}"
        if label == f"{joint_name} speed":
            return f"joint_speed_{index}"
        if label == f"{joint_name} acceleration":
            return f"joint_acceleration_{index}"
    for color_name in ("panel", "accent"):
        for channel, channel_name in enumerate(("R", "G", "B")):
            if label == f"{color_name.title()} {channel_name}":
                return f"color_{color_name}_{channel}"
    return None

def draw_value_box(text, x, y, field_id):
    panel_width = settings_panel_geometry()[2]
    width = max(80, min(145, panel_width - 265))
    active = text_edit_field == field_id
    draw_rect(x, y - 10, width, 25, color=(0.12, 0.15, 0.20) if not active else (0.25, 0.30, 0.38))
    draw_text(text_edit_buffer if active else text, x + 6, y - 6, 15, (255, 255, 255))
...
def draw_slider(label, value, low, high, x, y, width=230):
    ...
    draw_rect(x, y, width * max(0.0, min(1.0, fraction)), 7, color=tuple(settings["colors"]["accent"]))
    field_id = slider_field_id(label)
    if field_id is not None:
        draw_value_box(f"{value:.2f}", x + width + 20, y, field_id)

SETTINGS_CONTENT_HEIGHT = 2050

def settings_panel_geometry():
    panel_y = 70
    available_width = split_position()
    panel_width = min(520, max(360, available_width - 30))
    panel_x = max(15, (available_width - panel_width) // 2)
    panel_height = max(300, display_y - panel_y - 15) if settings_panel else 265
    return panel_x, panel_y, panel_width, panel_height

def max_settings_scroll(panel_height):
    return max(0, SETTINGS_CONTENT_HEIGHT - panel_height)

def draw_control_panel(target, angles, selected_axis, selected_joint, ik_enabled,
                       settings, pose_name, settings_panel):
    panel_x, panel_y, panel_width, panel_height = settings_panel_geometry()
    draw_rect(panel_x, panel_y, panel_width, panel_height, color=tuple(settings["colors"]["panel"]))
    content_y = panel_y - settings_scroll
    glEnable(GL_SCISSOR_TEST)
    glScissor(panel_x, display_y - panel_y - panel_height, panel_width, panel_height)
    draw_text("ROBOT ARM CONTROL", panel_x + 15, content_y + 12, 26, (255, 220, 120))
    draw_text("MODE: END EFFECTOR POSITION", panel_x + 15, content_y + 45, 20, (210, 230, 245))

    target_color = (255, 220, 120) if ik_enabled else (180, 190, 205)
    draw_text(
        f"TARGET {selected_axis.upper()}: {target[selected_axis]: .2f}  [X/Y/Z]",
        panel_x + 15, content_y + 76, 20, target_color
    )
    draw_text("J/L + I/K: move target", panel_x + 15, content_y + 102, 18, (180, 190, 205))
    draw_text("Shift + keys: fine adjustment", panel_x + 15, content_y + 125, 18, (180, 190, 205))
    draw_text(f"JOINT: {joint_names[selected_joint]}  [1-4] select", panel_x + 15, content_y + 158, 20, (210, 230, 245))
    draw_text(
        f"ANGLES: {angles[0]: .0f} {angles[1]: .0f} {angles[2]: .0f} {angles[3]: .0f}",
        panel_x + 15, content_y + 184, 18, (180, 190, 205)
    )
    draw_text("[I] IK [R] reset [F2] settings [F5] save [F7] load", panel_x + 15, content_y + 218, 17, (180, 190, 205))
    draw_text(f"CONTROL: {CONTROL_HZ:.0f} Hz   POSE: {pose_name}", panel_x + 15, content_y + 242, 17, (180, 190, 205))
    if settings_panel:
        draw_text("SETTINGS (drag sliders, scroll)", panel_x + 15, content_y + 275, 20, (255, 220, 120))
        draw_slider("P", settings["pid"]["p"][selected_joint], 0, 30, panel_x + 15, content_y + 315)
        draw_slider("I", settings["pid"]["i"][selected_joint], 0, 5, panel_x + 15, content_y + 355)
        draw_slider("D", settings["pid"]["d"][selected_joint], 0, 5, panel_x + 15, content_y + 395)
        draw_slider("Max speed", settings["max_joint_speed"], 5, 360, panel_x + 15, content_y + 435)
        draw_slider("Posture bias", settings["posture_bias"], 0, 1, panel_x + 15, content_y + 475)
        draw_slider("Angle weight", settings.get("angle_weight", 0.1), 0, 1, panel_x + 15, content_y + 515)
        draw_slider("Target X", target["x"], -20, 20, panel_x + 15, content_y + 555)
        draw_slider("Target Y", target["y"], -1, 15, panel_x + 15, content_y + 595)
        draw_slider("Target Z", target["z"], -20, 10, panel_x + 15, content_y + 635)
        draw_slider("End roll", target_orientation["roll"], -180, 180, panel_x + 15, content_y + 675)
        draw_slider("End pitch", target_orientation["pitch"], -180, 180, panel_x + 15, content_y + 715)
        draw_slider("End yaw", target_orientation["yaw"], -180, 180, panel_x + 15, content_y + 755)
        draw_text("MOTION LIMITS (degrees)", panel_x + 15, content_y + 775, 18, (255, 220, 120))
        for index, joint_name in enumerate(joint_names):
            low, high = settings["joint_limits"][index]
            row_y = content_y + 805 + index * 80
            draw_slider(f"{joint_name} min", low, -360, 360, panel_x + 15, row_y)
            draw_slider(f"{joint_name} max", high, -360, 360, panel_x + 15, row_y + 40)
        draw_slider("PID integral limit", settings["pid"]["integral_limit"], 0, 30, panel_x + 15, content_y + 1145)
        draw_slider("PID output limit", settings["pid"]["output_limit"], 1, 360, panel_x + 15, content_y + 1185)
        draw_text("MOTION PROFILE (per motor)", panel_x + 15, content_y + 1225, 18, (255, 220, 120))
        for index, joint_name in enumerate(joint_names):
            row_y = content_y + 1255 + index * 80
            draw_slider(
                f"{joint_name} speed",
                settings["joint_max_speed"][index],
                1,
                360,
                panel_x + 15,
                row_y,
            ...
            draw_slider(
                f"{joint_name} acceleration",
                settings["joint_acceleration"][index],
                1,
                720,
                panel_x + 15,
                row_y + 40,
            )
        draw_text("SAVE DIRECTORY", panel_x + 15, content_y + 1615, 18, (255, 220, 120))
        draw_value_box(save_directory, panel_x + 15, content_y + 1650, "save_directory")
        draw_text("APPEARANCE", panel_x + 15, content_y + 1715, 18, (255, 220, 120))
        for color_index, color_name in enumerate(("panel", "accent")):
            for channel, channel_name in enumerate(("R", "G", "B")):
                row_y = content_y + 1755 + (color_index * 3 + channel) * 40
                draw_slider(
                    f"{color_name.title()} {channel_name}",
                    settings["colors"][color_name][channel],
                    0,
                    1,
                    panel_x + 15,
                    row_y,
...
def save_current_settings():
    ...
    save_json(Path(save_directory).expanduser() / "settings.json", settings)
...
def save_current_pose(name):
    ...
    save_json(Path(save_directory).expanduser() / "poses.json", saved)
    return saved
...
def slider_id_at(relative_y):
    rows = [
        (297, 325, "p_selected"), (337, 365, "i_selected"),
        (377, 405, "d_selected"), (417, 445, "max_speed"),
        (457, 485, "posture_bias"), (497, 525, "angle_weight"),
        (537, 565, "target_x"), (577, 605, "target_y"),
        (617, 645, "target_z"), (657, 685, "orientation_roll"),
        (697, 725, "orientation_pitch"), (737, 765, "orientation_yaw"),
        (1127, 1155, "integral_limit"), (1167, 1195, "output_limit"),
        (1587, 1675, "save_directory"),
    ]
    for start, end, field_id in rows:
        if start <= relative_y <= end:
            return field_id
    for index in range(4):
        row = 787 + index * 80
        if row <= relative_y <= row + 28:
            return f"limit_low_{index}"
        if row + 40 <= relative_y <= row + 68:
            return f"limit_high_{index}"
        row = 1237 + index * 80
        if row <= relative_y <= row + 28:
            return f"joint_speed_{index}"
    ...
    for color_index, color_name in enumerate(("panel", "accent")):
        for channel in range(3):
            row = 1737 + (color_index * 3 + channel) * 40
            if row <= relative_y <= row + 28:
                return f"color_{color_name}_{channel}"
    return None

def field_value_text(field_id):
    if field_id == "save_directory":
        return save_directory
    values = {
        "p_selected": settings["pid"]["p"][selected_joint],
        "i_selected": settings["pid"]["i"][selected_joint],
        "d_selected": settings["pid"]["d"][selected_joint],
        "max_speed": settings["max_joint_speed"],
        "posture_bias": settings["posture_bias"],
        "angle_weight": settings["angle_weight"],
        "target_x": target_position["x"], "target_y": target_position["y"],
        "target_z": target_position["z"],
        "orientation_roll": target_orientation["roll"],
        "orientation_pitch": target_orientation["pitch"],
        "orientation_yaw": target_orientation["yaw"],
        "integral_limit": settings["pid"]["integral_limit"],
        "output_limit": settings["pid"]["output_limit"],
    }
    for index in range(4):
        values[f"limit_low_{index}"] = settings["joint_limits"][index][0]
        values[f"limit_high_{index}"] = settings["joint_limits"][index][1]
        values[f"joint_speed_{index}"] = settings["joint_max_speed"][index]
        values[f"joint_acceleration_{index}"] = settings["joint_acceleration"][index]
    for color_name in ("panel", "accent"):
        for channel in range(3):
            values[f"color_{color_name}_{channel}"] = settings["colors"][color_name][channel]
    return f"{values.get(field_id, 0.0):.4f}"

def apply_text_value(field_id, text):
    global save_directory, posture_bias, angle_weight, joint_limits
    if field_id == "save_directory":
        if text.strip():
            save_directory = text.strip()
        return
    try:
        value = float(text)
    except ValueError:
        return
    ranges = {
        "p_selected": (0, 30), "i_selected": (0, 5), "d_selected": (0, 5),
        "max_speed": (5, 360), "posture_bias": (0, 1), "angle_weight": (0, 1),
        "target_x": (-20, 20), "target_y": (-1, 15), "target_z": (-20, 10),
        "orientation_roll": (-180, 180), "orientation_pitch": (-180, 180),
        "orientation_yaw": (-180, 180), "integral_limit": (0, 30),
        "output_limit": (1, 360),
    }
    for index in range(4):
        ranges[f"limit_low_{index}"] = (-360, 360)
        ranges[f"limit_high_{index}"] = (-360, 360)
        ranges[f"joint_speed_{index}"] = (1, 360)
        ranges[f"joint_acceleration_{index}"] = (1, 720)
    for color_name in ("panel", "accent"):
        for channel in range(3):
            ranges[f"color_{color_name}_{channel}"] = (0, 1)
    low, high = ranges[field_id]
    value = max(low, min(high, value))
    if field_id in ("p_selected", "i_selected", "d_selected"):
        key = field_id[0]
        settings["pid"][key][selected_joint] = value
        controller = pid_controllers[selected_joint]
        controller.kp = settings["pid"]["p"][selected_joint]
        controller.ki = settings["pid"]["i"][selected_joint]
        controller.kd = settings["pid"]["d"][selected_joint]
    elif field_id == "max_speed":
        settings["max_joint_speed"] = value
    elif field_id == "posture_bias":
        posture_bias = value
        settings["posture_bias"] = value
    elif field_id == "angle_weight":
        angle_weight = value
        settings["angle_weight"] = value
    elif field_id.startswith("target_"):
        target_position[field_id[7:]] = value
    elif field_id.startswith("orientation_"):
        target_orientation[field_id[12:]] = value
    elif field_id == "integral_limit":
        settings["pid"][field_id] = value
        for controller in pid_controllers:
            controller.integral_limit = value
    elif field_id == "output_limit":
        settings["pid"][field_id] = value
        for controller in pid_controllers:
            controller.output_limit = value
    elif field_id.startswith("limit_"):
        index = int(field_id.rsplit("_", 1)[1])
        side = 0 if "low" in field_id else 1
        settings["joint_limits"][index][side] = value
        settings["joint_limits"][index].sort()
        joint_limits = tuple(tuple(pair) for pair in settings["joint_limits"])
    elif field_id.startswith("joint_speed_"):
        settings["joint_max_speed"][int(field_id.rsplit("_", 1)[1])] = value
    elif field_id.startswith("joint_acceleration_"):
        settings["joint_acceleration"][int(field_id.rsplit("_", 1)[1])] = value
    elif field_id.startswith("color_"):
        _, color_name, channel = field_id.split("_")
        settings["colors"][color_name][int(channel)] = value

def begin_text_edit(field_id):
    global text_edit_field, text_edit_buffer
    text_edit_field = field_id
    text_edit_buffer = field_value_text(field_id)
    pygame.key.start_text_input()

def commit_text_edit():
    global text_edit_field, text_edit_buffer
    if text_edit_field is not None:
        apply_text_value(text_edit_field, text_edit_buffer)
    text_edit_field = None
    text_edit_buffer = ""
...
def update_slider_value(slider, mouse_x):
    ...
    elif slider.startswith("limit_low_") or slider.startswith("limit_high_"):
        limit = set_slider_value(mouse_x, 30, 230, -360, 360)
        index = int(slider.rsplit("_", 1)[1])
        limits = settings["joint_limits"][index]
        if slider.startswith("limit_low_"):
            limits[0] = min(limit, limits[1])
        else:
            limits[1] = max(limit, limits[0])
        globals()["joint_limits"] = tuple(
            (float(pair[0]), float(pair[1]))
            for pair in settings["joint_limits"]
        )
    elif slider == "integral_limit":
        settings["pid"]["integral_limit"] = set_slider_value(mouse_x, 30, 230, 0, 30)
        for controller in pid_controllers:
            controller.integral_limit = settings["pid"]["integral_limit"]
    elif slider == "output_limit":
        settings["pid"]["output_limit"] = set_slider_value(mouse_x, 30, 230, 1, 360)
        for controller in pid_controllers:
            controller.output_limit = settings["pid"]["output_limit"]
    elif slider.startswith("joint_speed_"):
        index = int(slider.rsplit("_", 1)[1])
        settings["joint_max_speed"][index] = set_slider_value(mouse_x, 30, 230, 1, 360)
    elif slider.startswith("joint_acceleration_"):
        index = int(slider.rsplit("_", 1)[1])
        settings["joint_acceleration"][index] = set_slider_value(mouse_x, 30, 230, 1, 720)
    elif slider.startswith("color_"):
        _, color_name, channel = slider.split("_")
        settings["colors"][color_name][int(channel)] = set_slider_value(mouse_x, 30, 230, 0, 1)
...
SAVE_DIRECTORY = Path(__file__).resolve().parent
save_directory = str(SAVE_DIRECTORY)
settings_path = SAVE_DIRECTORY / "settings.json"
poses_path = SAVE_DIRECTORY / "poses.json"
settings = load_settings(settings_path)
joint_limits = tuple(
    (float(pair[0]), float(pair[1])) for pair in settings["joint_limits"]
)
posture_bias = settings["posture_bias"]
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.VIDEORESIZE:
            ...
            _, _, _, panel_height = settings_panel_geometry()
            settings_scroll = min(settings_scroll, max_settings_scroll(panel_height))
            continue
        if event.type == pygame.MOUSEWHEEL:
            ...
            if (
                settings_panel
                and panel_x <= mouse_x <= panel_x + panel_width
                and panel_y <= mouse_y <= panel_y + panel_height
            ):
                settings_scroll = max(
                    0,
                    min(
                        max_settings_scroll(panel_height),
                        settings_scroll - event.y * 60,
                    ),
                )
            continue
        if event.type == pygame.TEXTINPUT and text_edit_field is not None:
            text_edit_buffer += event.text
            continue
        if event.type == pygame.KEYDOWN and text_edit_field is not None:
            if event.key == K_RETURN or event.key == K_KP_ENTER:
                ...
                text_edit_buffer = text_edit_buffer[:-1]
                ...
                try:
                    ...
                    if pasted:
                        ...
                        text_edit_buffer += pasted.replace("\x00", "")
                except pygame.error:
                    pass
            continue
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button in (4, 5):
                continue
            panel_x, panel_y, panel_width, panel_height = settings_panel_geometry()
            if abs(event.pos[0] - split_position()) <= 10:
                split_drag = True
                slider_drag = None
                camera_control_enabled = False
                pygame.event.set_grab(False)
                pygame.mouse.set_visible(True)
                continue
            clicked_settings_panel = (
                panel_x <= event.pos[0] <= panel_x + panel_width
                and panel_y <= event.pos[1] <= panel_y + panel_height
            )
            if clicked_settings_panel and event.button == 1:
                slider_panel_y = panel_y - settings_scroll
                field_id = slider_id_at(event.pos[1] - slider_panel_y)
                value_box_start = panel_x + 250
                if field_id == "save_directory" or (
                    field_id is not None and event.pos[0] >= value_box_start
                ):
                    begin_text_edit(field_id)
                    camera_control_enabled = False
                    pygame.event.set_grab(False)
                    pygame.mouse.set_visible(True)
                    continue
                if slider_panel_y + 297 <= event.pos[1] <= slider_panel_y + 325:
                    slider_drag = "p_selected"
                elif slider_panel_y + 337 <= event.pos[1] <= slider_panel_y + 365:
                    slider_drag = "i_selected"
                elif slider_panel_y + 377 <= event.pos[1] <= slider_panel_y + 405:
                    slider_drag = "d_selected"
                elif slider_panel_y + 417 <= event.pos[1] <= slider_panel_y + 445:
                    slider_drag = "max_speed"
                elif slider_panel_y + 457 <= event.pos[1] <= slider_panel_y + 485:
                    slider_drag = "posture_bias"
                elif slider_panel_y + 497 <= event.pos[1] <= slider_panel_y + 525:
                    slider_drag = "angle_weight"
                elif slider_panel_y + 537 <= event.pos[1] <= slider_panel_y + 565:
                    slider_drag = "target_x"
                elif slider_panel_y + 577 <= event.pos[1] <= slider_panel_y + 605:
                    slider_drag = "target_y"
                elif slider_panel_y + 617 <= event.pos[1] <= slider_panel_y + 645:
                    slider_drag = "target_z"
                elif slider_panel_y + 657 <= event.pos[1] <= slider_panel_y + 685:
                    slider_drag = "orientation_roll"
                elif slider_panel_y + 697 <= event.pos[1] <= slider_panel_y + 725:
                    slider_drag = "orientation_pitch"
                elif slider_panel_y + 737 <= event.pos[1] <= slider_panel_y + 765:
                    slider_drag = "orientation_yaw"
                elif slider_panel_y + 787 <= event.pos[1] <= slider_panel_y + 815:
                    slider_drag = f"limit_low_{0}"
                elif slider_panel_y + 827 <= event.pos[1] <= slider_panel_y + 855:
                    slider_drag = f"limit_high_{0}"
                elif slider_panel_y + 867 <= event.pos[1] <= slider_panel_y + 895:
                    slider_drag = f"limit_low_{1}"
                elif slider_panel_y + 907 <= event.pos[1] <= slider_panel_y + 935:
                    slider_drag = f"limit_high_{1}"
                elif slider_panel_y + 947 <= event.pos[1] <= slider_panel_y + 975:
                    slider_drag = f"limit_low_{2}"
                elif slider_panel_y + 987 <= event.pos[1] <= slider_panel_y + 1015:
                    slider_drag = f"limit_high_{2}"
                elif slider_panel_y + 1027 <= event.pos[1] <= slider_panel_y + 1055:
                    slider_drag = f"limit_low_{3}"
                elif slider_panel_y + 1067 <= event.pos[1] <= slider_panel_y + 1095:
                    slider_drag = f"limit_high_{3}"
                elif slider_panel_y + 1127 <= event.pos[1] <= slider_panel_y + 1155:
                    slider_drag = "integral_limit"
                elif slider_panel_y + 1167 <= event.pos[1] <= slider_panel_y + 1195:
                    slider_drag = "output_limit"
                elif slider_panel_y + 1237 <= event.pos[1] <= slider_panel_y + 1265:
                    slider_drag = "joint_speed_0"
                elif slider_panel_y + 1277 <= event.pos[1] <= slider_panel_y + 1305:
                    slider_drag = "joint_acceleration_0"
                elif slider_panel_y + 1317 <= event.pos[1] <= slider_panel_y + 1345:
                    slider_drag = "joint_speed_1"
                elif slider_panel_y + 1357 <= event.pos[1] <= slider_panel_y + 1385:
                    slider_drag = "joint_acceleration_1"
                elif slider_panel_y + 1397 <= event.pos[1] <= slider_panel_y + 1425:
                    slider_drag = "joint_speed_2"
                elif slider_panel_y + 1437 <= event.pos[1] <= slider_panel_y + 1465:
                    slider_drag = "joint_acceleration_2"
                elif slider_panel_y + 1477 <= event.pos[1] <= slider_panel_y + 1505:
                    slider_drag = "joint_speed_3"
                elif slider_panel_y + 1517 <= event.pos[1] <= slider_panel_y + 1545:
                    slider_drag = "joint_acceleration_3"
                if slider_drag:
        ...
        if event.type == pygame.MOUSEMOTION and slider_drag:
            update_slider_value(slider_drag, event.pos[0])
        if event.type == pygame.MOUSEMOTION and split_drag:
            split_ratio = max(0.25, min(0.75, event.pos[0] / max(1, display_x)))
            update_render_viewport()
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
            split_drag = False
    ...
    for _ in range(control_scheduler.advance(elapsed)):
        ...
        for index, controller in enumerate(pid_controllers):
            ...
            target_angles[index] = max(low, min(high, target_angles[index]))
            speed_limit = min(
                settings["max_joint_speed"],
                settings["joint_max_speed"][index],
            )
            desired_velocity = controller.update(
                target_angles[index], joint_angles[index], CONTROL_DT
            )
            desired_velocity = max(
                -speed_limit,
                min(speed_limit, desired_velocity),
            )
            joint_velocity[index] = approach(
                joint_velocity[index],
                desired_velocity,
                settings["joint_acceleration"][index] * CONTROL_DT,
            )
            joint_angles[index] = approach(
                joint_angles[index],
                joint_angles[index] + joint_velocity[index] * CONTROL_DT,
                speed_limit * CONTROL_DT,
            )
            joint_angles[index] = max(low, min(high, joint_angles[index]))
...
#run once when setup stuff
    ...
    draw_2d_start()
    draw_rect(split_position() - 2, 0, 4, display_y, color=(0.85, 0.62, 0.18))

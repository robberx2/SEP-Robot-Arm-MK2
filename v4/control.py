...
def load_settings(path):
    ...
    for key, fallback, minimum in (
        ("joint_max_speed", 90.0, 1.0),
        ("joint_acceleration", 180.0, 1.0),
    ):
        values = settings.get(key)
        if not isinstance(values, list):
            values = DEFAULT_SETTINGS[key]
        settings[key] = [
            _number(item, fallback, minimum) for item in values[:4]
        ]
        settings[key] += [fallback] * (4 - len(settings[key]))
    ...
    for color_name in ("panel", "accent"):
        values = settings.get("colors", {}).get(color_name)
        if not isinstance(values, list):
            values = DEFAULT_SETTINGS["colors"][color_name]
        settings["colors"][color_name] = [
            _number(item, fallback, 0.0) for item, fallback in zip(
                values[:3], DEFAULT_SETTINGS["colors"][color_name]
            )
        ]
        settings["colors"][color_name] += [0.0] * (3 - len(settings["colors"][color_name]))
        settings["colors"][color_name] = [
            min(1.0, item) for item in settings["colors"][color_name]
        ]
    limits = settings.get("joint_limits")
    if not isinstance(limits, list):
        limits = DEFAULT_SETTINGS["joint_limits"]
    normalized_limits = []
    for index in range(4):
        pair = limits[index] if index < len(limits) else DEFAULT_SETTINGS["joint_limits"][index]
        if not isinstance(pair, (list, tuple)) or len(pair) != 2:
            pair = DEFAULT_SETTINGS["joint_limits"][index]
        low = _number(pair[0], DEFAULT_SETTINGS["joint_limits"][index][0])
        high = _number(pair[1], DEFAULT_SETTINGS["joint_limits"][index][1])
        normalized_limits.append([min(low, high), max(low, high)])
    settings["joint_limits"] = normalized_limits
    for key in ("p", "i", "d"):
...
class PID:
    ...
    def update(self, target, measured, dt=CONTROL_DT):
        ...
        if self.previous_measurement is None:
            ...
            derivative = -(float(measured) - self.previous_measurement) / dt
        self.filtered_derivative = (
            0.2 * derivative + 0.8 * self.filtered_derivative
        )
        candidate_integral = self.integral + error * dt
        candidate_integral = max(
            -self.integral_limit, min(self.integral_limit, candidate_integral)
        )
        unsaturated = (
            self.kp * error
        ...
        if (
            self.ki == 0.0
            or abs(unsaturated) < self.output_limit
            or (output >= self.output_limit and error < 0.0)
            or (output <= -self.output_limit and error > 0.0)
        ):
            self.integral = candidate_integral
        self.previous_error = error
        self.previous_measurement = float(measured)
        return output
